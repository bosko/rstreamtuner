...
Although RST is not so fast as streamtuner it is much more flexible.

== Dependencies

RST uses WxRuby as GUI toolkit. It is chosen primarily because of its simple interface
and multiplatfom support. For data processing it uses Nokogiri.

So in order to start RST you must install these two gems:
...
If you want to know about RST check:
